...
public class ExpenseService {
    ...
    private static Expense buildExpense(CreateExpenseDto request, Map<Long, User> idToUsers) {
        ...
        expense.setPaidBy(fromUserIds(request.getPaidBy(), idToUsers)); // Removed extra (
        expense.setOwedBy(fromUserIds(request.getOwedBy(), idToUsers));
        return expense;
    }

    // Change return type from Map<User, Long> to Map<User, Double>
    private static Map<User, Double> fromUserIds(Map<Long, Long> users, Map<Long, User> idToUsers) {
